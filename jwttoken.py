...
oauth2_scheme = OAuth2PasswordBearer(tokenUrl="/auth/login")  # FastAPI uses this to look for the JWT token in the Authorization: Bearer <token> header (for Swagger UI button only)
...
def verify_token(credentials: HTTPAuthorizationCredentials = Depends(HTTPBearer())):
    ...
    try:
        ...
        raise credentials_exception
    return {"user_id": user_id, "email": email}  # returns both user ID and email

# decodes the JWT manually
def decodeJWT(jwtoken: str):
    try:
        payload = jwt.decode(jwtoken, SECRET_KEY, algorithms=[ALGORITHM])  # decodes the JWT token using the secret key and algorithm
