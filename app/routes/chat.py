...
def ask_medical_question(q: ChatRequest, user_email: str = Depends(verify_token)):
...
@router.get("/get_previous_chat")
def get_previous_chat(user_email: str = Depends(verify_token)):
    chats = chat_collection.find({"user_id": user_email}).sort("timestamp", -1).limit(20)  # last 20 chats
    ...
    return {"user_id": user_email, "chat_history": chat_history}
